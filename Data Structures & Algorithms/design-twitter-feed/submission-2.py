class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        # then add in the newest tweet, putting in the newest tweeting persons 
        # next most recent tweet after each addition. 
        while minHeap and len(res) < 10:
            time, tweetId, followeeId, index = heapq.heappop(minHeap)
            res.append(tweetId)

            # Add this person's next older tweet if there are any
            if index >= 0:
                time, tweetId = self.posts[followeeId][index]
                heapq.heappush(
                    minHeap,
